...
class InsertableMatrix():
    ...
    def __init__(self, shape, dtype=np.float64):
        ...
        self.data = []
        self.nnz = 0
        util.logging.debug('Initiating insertable matrix with shape {} and data dtype {}.'.format(self.shape, self.data_dtype))
    ...
    def tocoo(self):
        util.logging.debug('Prepare coo matrix with {} entries and data dtype {}.'.format(len(self.data), self.data_dtype))
        assert len(self.row_indices) == len(self.colum_indices) == len(self.data)
        ...
        util.logging.debug('Returning matrix {!r}.'.format(matrix))
        assert matrix.dtype == self.data_dtype
        return matrix


    def asformat(self, format='csc'):
        util.logging.debug('Prepare {} matrix with {} entries and dtype {}.'.format(format, len(self.data), self.data_dtype))
