...
def cg(A, b, x0=None, tol=10**-5, maxiter=None):
    ...
    def callback(xk):
        ...
        #
        # util.logging.debug('Iteration {}: abs: avg diff = {:e}, averaged rel diff = {:e}, 2-norm abs diff = {:e}, 2-norm rel diff = {:e}'.format(k, diff_abs.mean(), diff_rel.mean(), np.linalg.norm(diff_abs, ord=2), np.linalg.norm(diff_rel, ord=2)))
        # xk_prev = np.copy(xk)
    ...
    if exit_code == 0:
        ...
        util.logging.warning('CG method exited. Tolerance {} not achieved after {} iterations. Exit code {}'.format(tol, k, exit_code))
        return x
    else:
        e = CGMethodError(exit_code)
        util.logging.error(str(e))
        raise e
