...
def generic(file, plot_function, font_size=20, transparent=True, caption=None, use_legend=False,
    ...
    if check_file(file, overwrite=overwrite):
        ...
        if tick_power_limit_scientific is not None:
            assert tick_power_limit_scientific_x is None
            assert tick_power_limit_scientific_y is None
            tick_power_limit_scientific_x = tick_power_limit_scientific
            tick_power_limit_scientific_y = tick_power_limit_scientific
        ...
        if tick_power_limit_scientific_y is not None and y_axis_move_scientific_notation_left:
            y_axis_move_scientific_notation(limit_scientific_notation=tick_power_limit_scientific_y)
...
def y_axis_move_scientific_notation(limit_scientific_notation, offs=0, dig=0, side='left', omit_last=False):
    ...
    if yoff >= limit_scientific_notation:
        ...
        # Plot the exponent
        plt.text(offs, .98, r'$\times10^{%i}$' % yoff, transform=plt.gca().transAxes, verticalalignment='top')
